...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    await msg.answer("Введите ваше имя (одно слово, до 12 символов):")
    await state.set_state(Registration.waiting_for_name)

@router.message(Registration.waiting_for_name)
async def set_name(msg: Message, state: FSMContext):
    name = msg.text.strip()
    if ' ' in name or len(name) > 12 or not re.match(r'^[a-zA-Zа-яА-Я]+$', name):
        await msg.answer("Имя должно быть одним словом и короче 12 символов, пошел нахуй, попробуй еще раз!")
        return
    if await add_user(msg.from_user.id, name):
